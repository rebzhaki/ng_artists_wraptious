...
import {ArtistsAuthStateModel, defaultUserAuthState} from "../models/auth.models";
...
export class AuthService {
  ...
  initialAuthState: ArtistsAuthStateModel = JSON.parse(localStorage.getItem(this.AUTHKEY)) ?? defaultUserAuthState;

  authStatus$ = new BehaviorSubject<ArtistsAuthStateModel>(this.initialAuthState);
  ...
  login(formData: {username: string, password: string}): Observable<ArtistsAuthStateModel> {
    const authValues = {
  ...
  logout(): Observable<unknown> {
    return this.apiAuthInvalidator().pipe(
      tap(() => {
        ...
        this.storage.setItem(this.AUTHKEY, JSON.stringify(defaultUserAuthState))
      }),
      share()
    );
  }
